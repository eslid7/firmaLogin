...
<script type="text/javascript">
	...
	function sbSendPortDataError(LvarTIP, LvarOK, LvarMSG)
	{
		if (LvarTIP == "sendPort")
		{
			...
			else
			{
				...
				document.form1.action = "<cfoutput>login.cfm?tkn=#url.tkn#&e=#url.e#&a=#url.a#&r=1</cfoutput>";
				<cfelse>
				document.form1.action = "login.cfm?r=1";
				</cfif>
	...
	function sbSubmit()
	{
	  ...
	  <cfif isdefined("url.tkn")>
		document.form1.action = "<cfoutput>login_ok.cfm?tkn=#url.tkn#&e=#url.e#&a=#url.a#</cfoutput>";
	  <cfelse>
		document.form1.action = "login_ok.cfm";
	  </cfif>
		document.form1.submit();
	}
	...
	// Se desconecta del WebSocket
		...
		// Aqui volver a pantalla de login
		if (window.confirm("Proceso de Autenticación con error:\n        " + pMsg + "\n\n¿Desea reintentar la Autenticación?"))
		{
			<cfif isdefined("url.tkn")>
			document.form1.action = "<cfoutput>login.cfm?tkn=#url.tkn#&e=#url.e#&a=#url.a#</cfoutput>";
			<cfelse>
			document.form1.action = "login.cfm";
			</cfif>
			document.form1.submit();
		}
