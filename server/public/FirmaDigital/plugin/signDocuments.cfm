...
<cfif URL.TKNTMP EQ "0">
	<cf_signDocuments_secondView_JWS args="#URL.ARGS#" functionName="#URL.FN#" onErrorfunctionName="#URL.errorFn#"></cf_signDocuments_secondView_JWS>
	<cfabort>
</cfif>
<!--- Pantalla a Nivel 2 --->
...
<cfif not isdefined("Application._FirmaDigital_.tokens.TK" & URL.TKNTMP & ".MSGDATA")>
	<script type="text/javascript">
		parent.parent.loadDigitalSignatureDocuments();
	</script>
	<cfabort>
</cfif>
...
<script type="text/javascript">
		...
		if (LvarTIP == "sendPort") {
			...
			if (!LvarOK) {
				...
				<cfif isdefined("URL.tkn")>
					document.form1.action = "<cfoutput>signDocuments.cfm?tkn=#URL.tkn#&a=*&e=RX&fn=#URL.FN#&errorFn=#URL.errorFn#&r=1</cfoutput>";
				<cfelse>
					document.form1.action = "signDocuments.cfm?r=1";
				</cfif>
	...
	function sbSubmit() {
		...
		parentWindow.signOnePerTurnDocuments = true;
